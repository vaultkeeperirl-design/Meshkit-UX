...
## 2025-03-01 - [Avoid O(N) String Copying Overhead and Main-Thread Blocking in WebSocket Handlers]
**Learning:** Found a critical performance bottleneck in `frontend/src/pages/ProcessLogs.jsx` where high-frequency WebSocket incoming messages were causing the component to update string state directly on every single message. Due to strings being immutable in JavaScript, `setLogs(prev => prev + event.data)` creates a completely new string object on every call. In high-velocity log streaming scenarios, this causes massive string copying overhead, memory reallocation churn, and forces excessive React render cycles that block the main thread and freeze the UI.
**Action:** Implemented an array buffer using `useRef` to store incoming chunks, combined with `requestAnimationFrame` to throttle state updates to ~60 FPS. This batches multiple WebSocket messages into a single string concatenation and state update cycle. Extracted the joined string `const newLogs = bufferRef.current.join("")` before triggering the React state update to avoid stale-closure bugs when resetting the buffer. Always remember to add a `useEffect` cleanup for `cancelAnimationFrame` to prevent memory leaks in unmounted components or background tabs.
